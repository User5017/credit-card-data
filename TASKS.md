...
| 2 | CFPB TCCP fetcher (semiannual xlsx, per card product) | raw table `tccp_products` + facts view for median max-APR by issuer tier; fixture test; golden entry from the CFPB page; first cross-source chart `v_offered_vs_paid` skeleton | done 2026-09-07 (six workbooks H1 2023 to H2 2025, 82 facts rows in 15 series; `data/raw/tccp/tccp_products.csv` 3,781 products; golden: 15 issuers above 30% in H1 2023 matches the CFPB's Feb 2024 report; `offered_vs_paid` chart live in Pricing; CI run pending at commit time) |
