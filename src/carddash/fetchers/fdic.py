...
- Charters come from crosswalks/issuers.csv, one row per FDIC certificate, with the merger date and the acquiring
  certificate for charters that no longer file. A merged-out charter's last call report is the last quarter end
  before the merger date (Discover Bank: merged 2025-05-18, last report 2025-03-31). Every active charter ends on
  the industry total's last quarter, so a charter that merges or fails without a crosswalk update turns the source
  red rather than quietly shortening one line. The issuer roll-up (acquirer plus acquired charters by period) is
  sql/views.sql v_fdic_issuer.
...
Outputs: raw_dir/latest/financials_<cert>.csv per charter and raw_dir/latest/aggregate_by_repdte.json, both the
response bodies byte for byte, then facts rows for six series per entity: CERT:<n> for each charter and
...
ISSUER_COLUMNS = [
    "issuer_id", "issuer_name", "fdic_cert", "bank_name", "kind", "sec_cik", "valid_from", "valid_to", "merged_into", "note",
]
ISSUER_TYPES = {  # schema for reading crosswalks/issuers.csv into DuckDB (render._connect), no type sniffing
    "issuer_id": "VARCHAR", "issuer_name": "VARCHAR", "fdic_cert": "INTEGER", "bank_name": "VARCHAR", "kind": "VARCHAR",
    "sec_cik": "VARCHAR", "valid_from": "DATE", "valid_to": "DATE", "merged_into": "INTEGER", "note": "VARCHAR",
}
...
def load_issuers(path: Path) -> pd.DataFrame:
    ...
    for c in ("issuer_id", "issuer_name", "fdic_cert", "bank_name", "kind"):
        n = int((df[c] == "").sum())
        if n:
            raise ValueError(f"{path.name}: {n} row(s) with an empty {c}")
    if not df["fdic_cert"].str.fullmatch(r"\d+").all():
        raise ValueError(f"{path.name}: fdic_cert must be a number: {sorted(df.loc[~df['fdic_cert'].str.fullmatch(r'\d+'), 'fdic_cert'])}")
    ...
    if (has_date != has_target).any():
        raise ValueError(f"{path.name}: valid_to and merged_into must be set together: certs {sorted(df.loc[has_date != has_target, 'fdic_cert'])}")
...
def download(session, params: dict, dest: Path, marker: bytes) -> None:
